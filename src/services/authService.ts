
import { API_BASE_URL } from '../constants';
import { User, AuthResponse } from '../types';

// Mock jwt-decode, in a real app, use the library
const jwtDecode = <T,>(token: string): T | null => {
  try {
    const base64Url = token.split('.')[1];
    const base64 = base64Url.replace(/-/g, '+').replace(/_/g, '/');
    const jsonPayload = decodeURIComponent(atob(base64).split('').map(function(c) {
        return '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2);
    }).join(''));
    return JSON.parse(jsonPayload) as T;
  } catch (error) {
    console.error("Failed to decode JWT", error);
    return null;
  }
};

const TOKEN_KEY = 'animeverse_token';

interface DecodedJwtPayload {
  id: number;
  nome: string;
  email: string;
  vip: boolean;
  admin: boolean;
  imagem_perfil?: string;
  is_banned?: boolean; // New: For ban status from JWT
  // banned_reason and admin_promotion_key_used are not typically in JWT for security/size.
  // They would be fetched from a dedicated user profile endpoint if needed client-side beyond admin panel.
}


export const authService = {
  login: async (userLogin: string, senhaLogin: string): Promise<{ token: string; user: User }> => {
    const response = await fetch(`${API_BASE_URL}/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userLogin, senha: senhaLogin }),
    });

    const data: AuthResponse = await response.json();

    if (!response.ok || !data.token) {
      let errorMessage = data.message || 'Login failed';
      if (response.status === 403 && data.reason) { // Check for banned reason in response
          errorMessage += ` Motivo: ${data.reason}`;
      }
      throw new Error(errorMessage);
    }
    
    localStorage.setItem(TOKEN_KEY, data.token);
    const decodedUser = jwtDecode<DecodedJwtPayload>(data.token);
    if (!decodedUser) {
        throw new Error('Failed to decode token');
    }
    
    const userObject: User = {
        id: decodedUser.id,
        nome: decodedUser.nome,
        email: decodedUser.email,
        vip: !!decodedUser.vip, 
        admin: !!decodedUser.admin, 
        imagem_perfil: decodedUser.imagem_perfil,
        is_banned: !!decodedUser.is_banned, // Add is_banned from token
        // banned_reason and admin_promotion_key_used are generally not in token,
        // but if they were, they would be populated here. They can be fetched if needed.
    };
    return { token: data.token, user: userObject };
  },

  register: async (userRegister: string, emailRegister: string, senhaRegister: string): Promise<AuthResponse> => {
    const response = await fetch(`${API_BASE_URL}/cadastro`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user: userRegister, email: emailRegister, senha: senhaRegister }),
    });
    const data: AuthResponse = await response.json();
    if (!response.ok) {
      throw new Error(data.message || 'Registration failed');
    }
    return data;
  },

  logout: (): void => {
    localStorage.removeItem(TOKEN_KEY);
  },

  getToken: (): string | null => {
    return localStorage.getItem(TOKEN_KEY);
  },

  getUserFromToken: (): User | null => {
    const token = localStorage.getItem(TOKEN_KEY);
    if (!token) return null;
    const decodedUser = jwtDecode<DecodedJwtPayload>(token);
    if (!decodedUser) return null;
    
    return {
        id: decodedUser.id,
        nome: decodedUser.nome,
        email: decodedUser.email,
        vip: !!decodedUser.vip,
        admin: !!decodedUser.admin,
        imagem_perfil: decodedUser.imagem_perfil,
        is_banned: !!decodedUser.is_banned, // Ensure is_banned is correctly populated
    };
  },
};
