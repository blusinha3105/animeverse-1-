...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      if (storedToken) {
        const currentUser = authService.getUserFromToken(); // This now correctly includes admin and is_banned status
        if (currentUser) {
          setUser(currentUser);
          setToken(storedToken);
        } else {
          // Invalid token or failed to decode
          authService.logout(); 
        }
      }
      setLoading(false);
  ...
  const login = async (userLogin: string, senhaLogin: string) => {
    setLoading(true);
    // login service now returns user object which includes is_banned and admin status
