  ...
  try {
    ...
    if (contentType && contentType.indexOf("application/json") !== -1) {
      ...
      if (response.status === 204 || response.headers.get('Content-Length') === '0') {
        return {} as T; // Return an empty object typed as T for 204 or empty body
      }
      // Handle non-JSON text responses if any endpoint returns them
      const textResponse = await response.text();
      // console.log(`API Non-JSON Response for ${requestUrl}:`, textResponse);
      return textResponse as unknown as T; // This might need specific handling based on what text is expected
    }
  } catch (error) {
...
const ANIME_EXIBIR_BASE_URL = 'https://gerenciador-we98.onrender.com';
...
export const apiService = {
  ...
  // Admin Users API
  adminGetUsers: (token: string): Promise<User[]> => 
    fetchApi<User[]>('/admin/users', { token }),
  adminBanUser: (userId: number, reason: string, token: string): Promise<{ success: boolean; message: string }> =>
    fetchApi<{ success: boolean; message: string }>(`/admin/users/${userId}/ban`, { method: 'PUT', body: JSON.stringify({ reason }), token }),
  adminUnbanUser: (userId: number, token: string): Promise<{ success: boolean; message: string }> =>
    fetchApi<{ success: boolean; message: string }>(`/admin/users/${userId}/unban`, { method: 'PUT', token }),
  adminPromoteUser: (userId: number, masterPassword: string, token: string): Promise<{ success: boolean; message: string }> =>
    fetchApi<{ success: boolean; message: string }>(`/admin/users/${userId}/promote`, { method: 'PUT', body: JSON.stringify({ masterPassword }), token }),
  adminDemoteUser: (userId: number, masterPassword: string, token: string): Promise<{ success: boolean; message: string }> =>
    fetchApi<{ success: boolean; message: string }>(`/admin/users/${userId}/demote`, { method: 'PUT', body: JSON.stringify({ masterPassword }), token }),
};
